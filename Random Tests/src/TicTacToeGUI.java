...
public class TicTacToeGUI extends JFrame {
	...
	JFrame frame = new JFrame("TicTacToe"); // Global frame and grid button
											// variables
	...
	public TicTacToeGUI() // Tic tac default constructor which adds and dimensions
						// Jframe
	{
		super();
		frame.setSize(350, 450);
		frame.setDefaultCloseOperation(EXIT_ON_CLOSE); // Setting dimension of
														// Jframe and setting
														// parameters
	...
	private void initialize() // Initialize tic tac toe game board
	{
		...
		JPanel game = new JPanel(new GridLayout(3, 3)); // Create two more
														// panels with layouts
														// for buttons
		...
		menu.setPreferredSize(new Dimension(300, 50)); // Setting dimensions of
														// panels
		game.setPreferredSize(new Dimension(300, 300));

		mainPanel.add(menu, BorderLayout.NORTH); // Add two panels to the main
													// container panel
		mainPanel.add(game, BorderLayout.SOUTH);

		menu.add(start, BorderLayout.WEST); // Add both start/reset buttons to
		...
		for (int i = 0; i < 3; i++) // Create grid of buttons for tic tac toe
									// game
		{
			for (int j = 0; j < 3; j++) {
	...
	private class myActionListener implements ActionListener { // Implementing
																// action
																// listener for
																// buttons
	...
	public static void main(String[] args) {
		TicTacToeGUI game = new TicTacToeGUI(); // main method and instantiating tic
											// tac
											// object and calling initialize
											// function
		game.initialize();
	}
}